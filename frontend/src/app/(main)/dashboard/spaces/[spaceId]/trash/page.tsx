...
export default function SpaceTrashPage() {
  ...
  const { data: foldersData, isLoading: foldersLoading } = useQuery({
    queryKey: ["drive-trash-folders", spaceId],
    queryFn: () => foldersApi.list({ spaceId, trashed: true, limit: 100 }),
  });

  const { data: filesData, isLoading: filesLoading } = useQuery({
    queryKey: ["drive-trash-files", spaceId],
    queryFn: () => filesApi.list({ spaceId, trashed: true, limit: 100 }),
  });

  const restoreFolder = useMutation({
    mutationFn: (id: string) => foldersApi.restore(id),
    onSuccess: () => void qc.invalidateQueries({ queryKey: ["drive-trash-folders", spaceId] }),
  });
  ...
  const restoreFile = useMutation({
    ...
    onSuccess: () => void qc.invalidateQueries({ queryKey: ["drive-trash-files", spaceId] }),
  });
  ...
  const emptyTrash = useMutation({
    mutationFn: async () => {
      await Promise.all([filesApi.emptyTrash(spaceId), foldersApi.emptyTrash(spaceId)]);
    },
    onSuccess: () => {
      toast.success(t("emptyTrash"));
      void qc.invalidateQueries({ queryKey: ["drive-trash-folders", spaceId] });
      void qc.invalidateQueries({ queryKey: ["drive-trash-files", spaceId] });
    },
  });
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-semibold">{tp("trash")}</h1>
        {!isEmpty && (
          <Button
            variant="outline"
            size="sm"
