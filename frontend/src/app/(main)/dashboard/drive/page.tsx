...
import { NewDocButton } from "./_components/new-doc-button";
...
import { RenameFileDialog } from "./_components/rename-file-dialog";
import { DriveItemRow } from "./_components/drive-item-row";
import { DriveBreadcrumb, type FolderCrumb } from "./_components/drive-breadcrumb";
...
export default function DrivePage() {
  ...
  const { data: foldersData, isLoading: foldersLoading } = useQuery({
    queryKey: ["drive-folders", currentFolderId, search],
    queryFn: () => foldersApi.list({ parentId: currentFolderId, search, limit: 100 }),
  });

  const { data: filesData, isLoading: filesLoading } = useQuery({
    queryKey: ["drive-files", currentFolderId, search],
    queryFn: () => filesApi.list({ folderId: currentFolderId, search, limit: 100 }),
  });

  const trashFolder = useMutation({
    mutationFn: (id: string) => foldersApi.trash(id),
    onSuccess: () => {
      void qc.invalidateQueries({ queryKey: ["drive-folders"] });
  ...
  const trashFile = useMutation({
    ...
    onSuccess: () => {
      void qc.invalidateQueries({ queryKey: ["drive-files"] });
      toast.success(t("moveToTrash"));
    },
  });
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <NewDocButton
            ...
            onCreated={() => void qc.invalidateQueries({ queryKey: ["drive-files"] })}
          />
      ...
      {/* Upload — Drawer on mobile, Dialog on desktop */}
      {isMobile ? (
        <Drawer open={showUpload} onOpenChange={setShowUpload}>
          <DrawerContent className="min-h-[50vh]">
            ...
            <div className="px-4 pb-6">
              <UploadZone folderId={currentFolderId} onUploadComplete={handleUploadComplete} />
            </div>
          </DrawerContent>
        </Drawer>
      ) : (
      ...
      <CreateFolderDialog
        ...
        onCreated={() => void qc.invalidateQueries({ queryKey: ["drive-folders"] })}
      />
